...
/**
 * Ultra-optimized MCMP strategy using Arena and MemorySegment for zero-copy, off-heap memory operations - similar to
 * Go's performance
 */
public class MCMPArenaStrategy extends ChunkReadStrategy implements Strategy {
    ...
    private List<Future<ChunkResult>> launchWorkers(String path, ExecutorService executor)
        ...
        chunks.forEach(
                (chunk) -> futures.add(executor.submit(() -> processChunk(chunk))));
        return futures;
    }
